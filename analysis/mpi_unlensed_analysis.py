...
def load_matched_catalogs_zs(f_lambda, Ns, ddir='/pscratch/sd/p/pecom/anacal_blends/', load_unrec=True,
    ...
    for j,i in enumerate(Ns):
        ...
        if load_unrec:
            # multiz_shear_bin = np.load(f'{pdir}/anacal_blends/unrec/{bin_dir_name}{bin_num}_unlensed_{i}_unrec_unlensed_{pix}pix.npy')
            ...
            unrec_blends_multiz = Table.read(f'{pdir}/anacal_blends/unrec/{bin_dir_name}{bin_num}_unlensed_{i}_{pix}pix_unrec.pq')
            unrec_filt_bin = np.array(unrec_blends_multiz['blend_diff'] < 1)


            # const_shear_bin = np.load(f'{pdir}/anacal_blends/unrec/{const_dir_name}_unlensed_{i}_unrec_unlensed_{pix}pix.npy')
            ...
            unrec_blends_const = Table.read(f'{pdir}/anacal_blends/unrec/{const_dir_name}_unlensed_{i}_{pix}pix_unrec.pq')
...
if rank==0:
    ...
    if test:
        if unrec:
            np.save(f'{hdir}/anacal_scripts/data/matched_bin{zbin}_unlensed_unrec_{pix}pix_test.npy', matched_proper)
        else:
            np.save(f'{hdir}/anacal_scripts/data/matched_bin{zbin}_unlensed_test.npy', matched_proper)
    else:
        if unrec:
            np.save(f'{hdir}/anacal_scripts/data/matched_bin{zbin}_unlensed_obsunrec_{pix}pix.npy', matched_proper)
        else:
            np.save(f'{hdir}/anacal_scripts/data/matched_bin{zbin}_unlensed_obs.npy', matched_proper)
